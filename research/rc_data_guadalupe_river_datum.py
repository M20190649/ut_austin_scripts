...
class RCDist(RCData):
	...
	def draw_xsect(self,save=False):
		...
		hand_xsect = scipy.array([[0,self.datum_values[0]]]) # gage datum shift
		...
		for h in range(len(self.handstage)):
			...
			neg = scipy.array([[-delta_w,h+self.datum_values[0]]])
			pos = scipy.array([[delta_w,h+self.datum_values[0]]])
		...
		plt.gca().set_ylim(bottom=self.datum_values[0],top=pos[-1][1]+1) # 1 above USGS height
...
if __name__ == '__main__':
	...
	# Use 191002 and 121003 for Guadalupe River in Guadalupe River Basin
	...
	idlookup = pandas.read_csv('usgsid_to_comid.csv',usecols=['SOURCE_FEA','FLComID'])
	...
	for comid in comids:
		try: 
			...
			rcdist.draw_xsect(save='results/guadalupe_river/xsect_datum_{0}'.format(str(comid)))
			...
			# rcdist.plot_rc(save='results/guadalupe_river/hand_vs_usgs_rc_{0}'.format(str(comid)),
				# save=False,
