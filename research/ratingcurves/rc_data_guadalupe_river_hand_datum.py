...
class RCDist(RCData):
	...
	def draw_xsect(self,save=False):
		...
		plt.gca().set_ylim(bottom=0,top=1800) # 1 above USGS height
...
if __name__ == '__main__':
	...
	hand_props = 'hydroprop-fulltable-121002.csv'
	...
	idlookup = pandas.read_csv('guad_blanco_river_data.csv',usecols=['SOURCE_FEA','FLComID','ned_10m'])
	...
	# ALL COMIDS IN GUADALUPE AND BLANCO RIVERS
	comids = [3586192,3585626,1630223,3587616,1631023,3589508,3589120,1619595,1620031,1631387,1631413,1620877,1622713,1637447,1639225,1640519]
	...
	for comid in comids:
		try: 
			...
			# rcdist.draw_xsect(save='results/guad_blanco_river/xsects_hand_datum_uniform/xsect_hand_datum_uniform_{0}'.format(str(comid)))
			...
			rcdist.plot_rc(save='results/guad_blanco_river/ratingcurves/hand_vs_usgs_rc_{0}'.format(str(comid)),
				# save=False,
