...
if __name__ == '__main__':
	...
	xsintersect = pandas.read_csv('onionck/hecras_xsintersect.csv',usecols=['COMID','XSID','ProfileM'],index_col=None,header=0)
	xsrating = pandas.read_csv('onionck/hecras_xsrating.csv',usecols=['Stage_Height_ft_','Discharge_cfs_','XSID'],header=0)
	# arraylist = []
	# print xsintersect.as_matrix()
	# arraylist.append(xsintersect.as_matrix())
	# arraylist.append(xsrating.as_matrix())
	# print arraylist
	# combined = scipy.vstack(arraylist)
	# finaldf = pandas.DataFrame(combined)
	# print finaldf

	# comid = idlookup['FLComID'][0]

	merged = pandas.merge(left=xsintersect,right=xsrating,left_on='XSID',right_on='XSID')

	print merged

	# print pandas.concat([xsintersect,xsrating],axis=1)


	# print xsintersect
	# print xsrating

	# frames = [xsintersect,xsrating]
	# frames = pandas.concat(frames,axis=1)
	# print frames
