...
def Shapefile_Attribute_Reader(catchmentshp, flowlineshp):
    ...
    for catchment in layer:
        catchment_COMID.append(catchment.GetField("OBJECTID"))
    ...
    for flowline in layer:
        flowline_COMID.append(flowline.GetField("OBJECTID"))
        length = flowline.GetField("LENGTHKM") * flowline.GetField("Shape_Leng") / 0.096491 # specific to Slaughter Flowline in Onion Creek
        RiverLength_dic[str(flowline.GetField("OBJECTID"))] = length
        Slope_dic[str(flowline.GetField("OBJECTID"))] = flowline.GetField("SLOPE")
...
def HANDClipper(catchmentshp, flowlineshp, handtif,
    ...
    for i in range(0, inLayer.GetFeatureCount()):
        ...
        COMID = inFeature.GetField('OBJECTID')
...
# python Hydraulic_Property_V2.2.py -catchment c1.shp -flowline f1.shp -HAND h1.tif -netCDF nc1.nc
