...
class RCDist(RCData):
	...
	def optimize_n(self):
		...
		for h in self.handstage: 
			# res = min(self.usgsh[0], key=lambda x:abs(x-h))
		...
		print area
		print hydrad
		print self.handslope
		print usgs_qlist
		return self.mannings_n(area=area,hydrad=hydrad,slope=self.handslope,disch=usgs_qlist)
		print '-------------------------------'		# print len(self.handh)
		# print self.handh
		# print len(self.handq)
		# print len(self.handstage)
		# print self.handq
		# print usgs_hlist
		# print self.handh[:len(usgs_hlist)]
		# print '---------------'
		# print usgs_qlist
		# print self.handq[:len(usgs_qlist)]
		# 	if res nums.append(res)
		# print zip(self.handh,nums)
		# print [h for h in self.usgsh[0] if (h % 1) == 0]
		# [h for h in self.usgsh if (h % 1) == 0]

		# area = self.handarea[:len(usgs_qlist)]
		# print len(self.handarea)
		# print self.handarea
		# rad = self.handrad[:len(usgs_qlist)]
...
if __name__ == '__main__':
	...
	for comid in comids:
		...
		rcdist.plot_rc(save=False,hand=True,usgs=True,sprnt=True,
