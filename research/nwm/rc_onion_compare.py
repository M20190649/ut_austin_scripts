...
class RCData:
	...
	def get_xs(self):
		...
		for i, rs in enumerate( self.xs['RiverStation'].unique() ):
			...
			# Find max stage value for plotting x_axis
...
class RCDist(RCData):
	...
	def plot_rc(self,hand=True,xs=True,ci=True,alpha=0.05,div=5):
		...
		# upper bounds
		f = scipy.interpolate.interp1d(self.ci_qvals,self.ubounds,kind='linear') # kind=cubic
		ax.plot(self.ci_qvals,f(self.ci_qvals),
			label='upper-bound',c='orange', linewidth=5)#,s=400,marker='^'
		# lower bounds
		f = scipy.interpolate.interp1d(self.ci_qvals,self.lbounds,kind='linear') # kind=cubic
		ax.plot(self.ci_qvals,f(self.ci_qvals),
			label='lower-bound',c='orange', linewidth=5)#,s=400,marker='^'

		# Plot linearly-interpolated HAND rating curve
		f = scipy.interpolate.interp1d(self.usgsq,self.usgsh,kind='linear') # kind=cubic
...
if __name__ == '__main__':
	...
	for i in range(len(idlookup)):
		...
		rcdist.plot_rc(hand=True,xs=True,ci=True,alpha=0.05,div=5)
