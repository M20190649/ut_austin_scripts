...
class RCDist(RCData):
	...
	def draw_xsect(self,save=False):
		...
		for usgsid in self.usgsids:
			...
			try: 
				title = 'COMID {0}, with GZF'.format(self.comid) # Update plot title to mention no GZF
...
if __name__ == '__main__':
	...
	idlookup = pandas.read_csv('onion_data.csv',usecols=['SOURCE_FEA','FLComID','ned_10m'])
	...
	# nhd = pandas.read_csv('oniondata/onion_nhd.csv',usecols=['COMID','GNIS_NAME'])
	# comids = nhd[nhd['GNIS_NAME'] == 'Onion Creek']['COMID'].values
	...
	# ALL COMIDS IN ONION CK
	# comids = [5781401, 5781417, 5781407, 5780099, 5781431, 5781477, 5781731, 5781373, 5781353, 5781345, 5781369, 5781393]
	...
	for comid in comids:
		try: 
			...
			rcdist.draw_xsect(save='results/thesis/xsects/xsects_{0}'.format(str(comid)))
			...
			# rcdist.plot_rc(
			# 	save='results/thesis/usgs/usgs_{0}'.format(str(comid)),
			# 	# save=False,
			# 	hand=False,usgs=True,#xs=True,
