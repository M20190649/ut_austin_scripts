...
class RCData:
	...
	def __init__(self, comid, hand_curves, hand_curves_idx, hand_props, 
		...
		self.comid = comid
		self.hand_curves = hand_curves
		self.hand_curves_idx = hand_curves_idx
		self.hand_props = hand_props
		self.hand_props_idx = hand_props_idx
		self.xs = xs
		self.idlookup = idlookup

	def get_hand_curves(self): 
		""" Initializes self.handq [cfs] and self.handh [ft]."""
		print self.hand_curves
...
class RCDist(RCData):
	...
	def get_ci(self,alpha=0.05,div=5,axis='x'):
		...
		if axis == 'x':
			...
			for h in ci_hvals: 
				other_dischs = self.get_disch(h) # xs dischs		
...
# # # # # TEST PLOT. Should be later removed. # # # # #
		...
		if xs: # Plot all linearly-interpolated XS rating curves
			# Get data
			self.hand_props = Dataset(self.hand_props,'r')
			hand_props_idx = pandas.read_csv(self.hand_props_idx)
			self.hand_props_idx = hand_props_idx.loc[ hand_props_idx['comid'] == \
				self.comid ]['index'].values[0]
			self.get_hand_props()
			self.xs = self.xs[ self.xs['COMID'] == str(self.comid)]
...
# ************
		...
		if usgs: # Plot interpolated USGS rating curve
			# Get data
			usgsids = self.idlookup.loc[self.idlookup['FLComID'] == \
				self.comid]['SOURCE_FEA'].values
			usgsrc = 'http://waterdata.usgs.gov/nwisweb/get_ratings?' \
		...
		if hand: # Plot interpolated HAND rating curve
			# Get data
			hand_curves = Dataset(self.hand_curves,'r')
			hand_curves_idx = pandas.read_csv(self.hand_curves_idx)
			hand_curves_idx = hand_curves_idx.loc[ hand_curves_idx['comid'] == \
				self.comid ]['index'].values[0]
		...
		plt.grid()
		if filename:
			fig.savefig('results/thesis/hecras/hecras_{0}.png'.format(self.comid))
		else: 
...
if __name__ == '__main__':
	...
	for comid in comids:
		...
		rcdist.plot_rc(filename=False,hand=True,usgs=False,xs=True,xsapprox=False,
			ci=False,raw=False,kind='linear',alpha=0.05,div=5)
